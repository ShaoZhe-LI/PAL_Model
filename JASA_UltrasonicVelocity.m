...
% - output 1D velocity-level figures and velocity-level-difference figures
...
SAVE_FIGS = true;   % <<< true 保存；false 不保存
...
for ic = 1:numel(vel_comp_list)
    ...
    % 只针对 m = 3 的 v_phi 误差图固定 y 轴范围
    if source.m == 3 && strcmpi(vel_comp, 'vphi')
...
LvK = 20*log10(max(abs(vK), v_floor) / v_ref);
LvD = 20*log10(max(abs(vD), v_floor) / v_ref);

diff_Lv = abs(LvD - LvK);

switch lower(strtrim(string(vel_comp)))
    case "vrho"
        ylab_level = '$\mathrm{L}_{v_\rho}\ (\mathrm{dB})$';
        ylab_diff  = '$|\Delta \mathrm{L}_{v_\rho}|\ (\mathrm{dB})$';
    case "vphi"
        ylab_level = '$\mathrm{L}_{v_\phi}\ (\mathrm{dB})$';
        ylab_diff  = '$|\Delta \mathrm{L}_{v_\phi}|\ (\mathrm{dB})$';
    case "vz"
        ylab_level = '$\mathrm{L}_{v_z}\ (\mathrm{dB})$';
        ylab_diff  = '$|\Delta \mathrm{L}_{v_z}|\ (\mathrm{dB})$';
    otherwise
        ylab_level = '$\mathrm{L}_v\ (\mathrm{dB})$';
        ylab_diff  = '$|\Delta \mathrm{L}_v|\ (\mathrm{dB})$';
end
...
Lv_for_ylim = [LvD(show_mask); LvK(show_mask)];
diff_for_ylim = diff_Lv(show_mask);
...
% ============================================================
% 1) VELOCITY LEVEL FIGURE
% ============================================================
f1 = figure('Position', fig_pos, 'Color', 'w');
ax1 = axes('Parent', f1, 'Units', 'normalized', 'Position', [0.12 0.24 0.84 0.60]);
hold(ax1, 'on');

plot(coord_vec, LvD, '-',  'LineWidth', lw_ref,  'Color', [0 0.2 1.0], ...
    'DisplayName', ref_name);
plot(coord_vec, LvK, '--', 'LineWidth', lw_main, 'Color', [1 0 0], ...
    'DisplayName', 'King');

set(ax1, 'XScale', 'log');
xlim(ax1, x_show);
local_set_y_margin_general(ax1, Lv_for_ylim);
box(ax1, 'on');
...
local_save_fig_all(f1, sprintf('King_vs_%s_%s_vlevel', ref_name, spec.file_tag));

% ============================================================
% 2) VELOCITY LEVEL DIFFERENCE FIGURE
% ============================================================
f2 = figure('Position', fig_pos, 'Color', 'w');
ax2 = axes('Parent', f2, 'Units', 'normalized', 'Position', [0.12 0.24 0.84 0.60]);
hold(ax2, 'on');

plot(coord_vec, diff_Lv, '-', 'LineWidth', 2.0, 'Color', [0 0.2 1.0]);
...
local_save_fig_all(f2, sprintf('King_vs_%s_%s_vleveldiff', ref_name, spec.file_tag));
end

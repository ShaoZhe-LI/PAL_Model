%% ============================================================
% MAIN: King vs DIM velocity (single call: method='both')
% + compare_mode = 'radial' / 'axial'
%
% radial:
%   - compare radial profile at z = z_target
%   - 1D compare on phi=0 line (y=0, x=rho): v_rho, v_phi, v_z
%   - 2D xOy @ z≈z_use (King vs DIM, via exp(j*m*theta) extension)
%
% axial:
%   - compare axial profile at rho = xh(1)
%   - 1D compare along z: v_rho, v_phi, v_z
%   - 2D xOy figures are skipped
%
% Notes:
% - Based on the first velocity script
% - Only added compare_mode and corresponding DIM observation-grid switch
% - Parameters are aligned to the second script as much as possible
%
% Dependencies:
...
source.a = 0.05;
source.v0 = 0.108;      % aligned to the second script
source.v_ratio = 1;
source.m = 3;
source.F = 0.2;
...
calc.dim.dis_coe = 16 * 1;                 % aligned to the second script
calc.dim.margin = 1;
calc.dim.src_discretization = 'polar';     % 'cart' or 'polar'
calc.dim.block_size = 200000;              % aligned to the second script
calc.dim.src_block_size = 5000;            % keep for velocity DIM implementation

% --- King analytic spectrum stability ---
calc.king.gspec_method = 'transform';      % 'analytic' or 'transform'
calc.king.eps_kzz   = 1e-3;
calc.king.eps_phase = calc.king.eps_kzz;
calc.king.kz_min    = 1e-12;

% --- band refine ---
calc.king.band_refine.enable = false;      % aligned to the second script

% --- ASM settings (kept for consistency; only used if underlying function needs them) ---
...
%% -------------------- target plane & ds --------------------
z_target = 1;
ds = 16 * 2;   % aligned to the second script
...
%% ============================================================
% Prepare DIM observation grid
% - radial: x = rho_ds, y = 0, z = z_use
% - axial : x = rho_full(1), y = 0, z = z_full
%% ============================================================
obs_grid = struct();
obs_grid.dim = struct();
...
%% ============================================================
% Extract King / DIM according to compare_mode
% ============================================================
zK   = resV.king.z(:);
rhoK = resV.king.rho(:);
...
switch lower(compare_mode)
    case 'radial'
        ...
        if szR(1) ~= Ny_line || szR(2) ~= Nx_line || szR(3) ~= Nz_line
            error(['DIM output size mismatch: expected (Ny=%d, Nx=%d, Nz=%d), ' ...
                   'got (%d,%d,%d).'], ...
        ...
        coord_label = '\rho (m)';
        coord_name  = 'rho';
        coord_title = sprintf('z = %.3f m', z_use);

        % radial mode: exclude rho=0 for 1D phase/error comparison
        mask_ok = coord_vec > 0;

    case 'axial'
        % ---------- King ----------
        rho_ax = rho_full(1);
        [~, iR] = min(abs(rhoK - rho_ax));

        vR_king = reshape(resV.king.v_rho_f1(iR, :).', [], 1);
        vP_king = reshape(resV.king.v_phi_f1(iR, :).', [], 1);
        vZ_king = reshape(resV.king.v_z_f1  (iR, :).', [], 1);
        ...
        vR_dim = reshape(resV.dim.v_rho_f1(1,1,:), [], 1);
        vP_dim = reshape(resV.dim.v_phi_f1(1,1,:), [], 1);
        vZ_dim = reshape(resV.dim.v_z_f1  (1,1,:), [], 1);
        ...
        % axial mode: keep all points
        mask_ok = isfinite(coord_vec);
end

%% ============================================================
% Metrics
% ============================================================
coord_p = coord_vec(mask_ok);
...
% -------------------- v_rho --------------------
figure('Name',sprintf('v_rho: King vs DIM @ %s', coord_title), ...
    'position',[100 100 1200 900]);

subplot(3,1,1);
plot(coord_p, abs(vRk), 'LineWidth',1.8); hold on;
plot(coord_p, abs(vRd), '--', 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
ylabel('$|v_\rho|$ (m/s)','Interpreter','latex');
title(sprintf('Magnitude: $v_\\rho$ @ %s', coord_title), 'Interpreter','latex');
legend('King','DIM','Location','best');
apply_axes_style(gca); fontsize(gca,22,'points');

subplot(3,1,2);
plot(coord_p, phR_k, 'LineWidth',1.8); hold on;
plot(coord_p, phR_d, '--', 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
...
subplot(3,1,3);
plot(coord_p, errR_log, 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
...
local_save_fig_png(gcf, sprintf('vRho_1D_compare3_%s_%s', coord_name, lower(compare_mode)));

% -------------------- v_phi --------------------
figure('Name',sprintf('v_phi: King vs DIM @ %s', coord_title), ...
    'position',[120 120 1200 900]);

subplot(3,1,1);
plot(coord_p, abs(vPk), 'LineWidth',1.8); hold on;
plot(coord_p, abs(vPd), '--', 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
ylabel('$|v_\phi|$ (m/s)','Interpreter','latex');
title(sprintf('Magnitude: $v_\\phi$ @ %s', coord_title), 'Interpreter','latex');
legend('King','DIM','Location','best');
apply_axes_style(gca); fontsize(gca,22,'points');

subplot(3,1,2);
plot(coord_p, phP_k, 'LineWidth',1.8); hold on;
plot(coord_p, phP_d, '--', 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
ylabel('Phase (rad)','Interpreter','latex');
title('Phase', 'Interpreter','latex');
...
subplot(3,1,3);
plot(coord_p, errP_log, 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
ylabel('$\log_{10}$ relative error','Interpreter','latex');
title('$\log_{10}(|v_{DIM}-v_{King}|/(|v_{King}|+\epsilon))$', 'Interpreter','latex');
...
local_save_fig_png(gcf, sprintf('vPhi_1D_compare3_%s_%s', coord_name, lower(compare_mode)));

% -------------------- v_z --------------------
figure('Name',sprintf('v_z: King vs DIM @ %s', coord_title), ...
    'position',[140 140 1200 900]);

subplot(3,1,1);
plot(coord_p, abs(vZk), 'LineWidth',1.8); hold on;
plot(coord_p, abs(vZd), '--', 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
ylabel('$|v_z|$ (m/s)','Interpreter','latex');
title(sprintf('Magnitude: $v_z$ @ %s', coord_title), 'Interpreter','latex');
legend('King','DIM','Location','best');
apply_axes_style(gca); fontsize(gca,22,'points');

subplot(3,1,2);
plot(coord_p, phZ_k, 'LineWidth',1.8); hold on;
plot(coord_p, phZ_d, '--', 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
ylabel('Phase (rad)','Interpreter','latex');
title('Phase', 'Interpreter','latex');
...
subplot(3,1,3);
plot(coord_p, errZ_log, 'LineWidth',1.8);
grid on;
xlabel(coord_label,'Interpreter','latex');
ylabel('$\log_{10}$ relative error','Interpreter','latex');
title('$\log_{10}(|v_{DIM}-v_{King}|/(|v_{King}|+\epsilon))$', 'Interpreter','latex');
...
local_save_fig_png(gcf, sprintf('vZ_1D_compare3_%s_%s', coord_name, lower(compare_mode)));
...
if strcmpi(compare_mode,'radial')
    fprintf('z_target = %.3f m, z_use = %.6f m\n', z_target, z_use);
else
...
%% ============================================================
% 2D xOy @ z≈1 m (King + DIM phase extension) — only for radial mode
%% ============================================================
if strcmpi(compare_mode,'radial')
    ...
    % King 1D lines
    vRk2 = vR_king(:).'; vRk2 = vRk2(1:idx_rb);
    vPk2 = vP_king(:).'; vPk2 = vPk2(1:idx_rb);
    vZk2 = vZ_king(:).'; vZk2 = vZk2(1:idx_rb);

    % DIM 1D lines
    vRd2 = vR_dim(:).';  vRd2 = vRd2(1:idx_rb);
    vPd2 = vP_dim(:).';  vPd2 = vPd2(1:idx_rb);
    vZd2 = vZ_dim(:).';  vZd2 = vZd2(1:idx_rb);
    ...
    vR2k = (vRk2(:) * ones(1, numel(theta_k))) .* Ephi_k;
    vP2k = (vPk2(:) * ones(1, numel(theta_k))) .* Ephi_k;
    vZ2k = (vZk2(:) * ones(1, numel(theta_k))) .* Ephi_k;
    ...
    vR2d = (single(vRd2(:)) * ones(1, size(THd,2), 'single')) .* Ephi_d;
    vP2d = (single(vPd2(:)) * ones(1, size(THd,2), 'single')) .* Ephi_d;
    vZ2d = (single(vZd2(:)) * ones(1, size(THd,2), 'single')) .* Ephi_d;

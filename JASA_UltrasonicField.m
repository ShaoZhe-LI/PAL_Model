...
SAVE_FIGS = false;   % <<< true 保存；false 不保存
...
rho_show_min = 1e-3;       % x-axis lower limit for radial profiles
...
% 只针对 source.m == 3 的径向误差图，固定 y 轴为 0~0.02 dB
% if source.m == 3
%     spec_rad.diff_ylim = [0, 0.02];
% end

local_plot_compare_1d_spl(spec_rad, pK_rad, pD_rad, medium, res_rad, fig);
...
if source.m == 0
    ...
    local_plot_compare_1d_spl(spec_line, pK_line, pD_line, medium, res_line, fig);
    ...
    [~, fht_line_tmp] = make_source_velocity(source, medium, calc_line); %#ok<ASGLU>
...
function local_plot_compare_1d_spl(spec, pK, pD, medium, res, fig) %#ok<INUSD>
